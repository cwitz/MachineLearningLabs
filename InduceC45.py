...
def main():
   ...
   if len(args) < 2 or len(args) > 3:
      print("Usage: python3 InduceC45.py <TrainingSetFile.csv> " +
            "[<restrictionsFile>]")
      exit() 
   ...
   try:
      if len(args) == 3:
         restrline = open(args[2], 'r').readlines()[0]
   ...
   decisionTree = c45(dataset, attributelist, threshold, classAttr, Ddataframe)
   jStr = "{\"dataset\":"
   jStr += "\"" + args[1] + "\""
   jStr += ",\n"
   jStr += treeToJson(decisionTree)
   jStr += "}"
   print(jStr)
